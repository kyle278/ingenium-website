    ...
    <div className="space-y-20 pb-8 md:space-y-28">
      <section className="grid items-start gap-8 pt-4 lg:grid-cols-[0.95fr,1.05fr]">
        <div>
          ...
          <p className="mt-6 max-w-[65ch] type-body-lead text-[var(--color-text-soft)]">{config.summary}</p>
          <div className="mt-8 grid gap-3">
            {config.facts.map((fact) => (
              <div key={fact} className="rounded-2xl border border-[var(--color-line)] bg-white/72 px-4 py-4 type-body-sm text-[var(--color-text-soft)]">
                {fact}
              </div>
            ))}
          </div>
        ...
        <SurfaceCard className="p-6 sm:p-8">
          ...
          <div className="mt-6 rounded-2xl bg-[var(--color-panel-low)] px-4 py-4 type-body-sm text-[var(--color-text-soft)]">
            {config.nextStep}
          </div>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          {config.faq.map((item) => (
            <SurfaceCard key={item.question} className="p-6">
              ...
              <p className="mt-3 type-body-sm text-[var(--color-text-soft)]">{item.answer}</p>
            </SurfaceCard>
          ))}
        </div>
      </section>
